...
import { Inbox, Ticket, AlertTriangle } from "lucide-react";
...
export default function TicketsWorkspace() {
  ...
  // Saved views — what agents actually work from. Requesters get a simpler set
  // scoped to the tickets they opened.
  const views = useMemo(() => (
    isAgent
      ? [
          { key: "mine",       label: "My tickets",  fn: (t) => t.assignee === me && t.statusKey !== "resolved" },
          { key: "unassigned", label: "Unassigned",  fn: (t) => t.assignee === "Unassigned" && t.statusKey !== "resolved" },
          { key: "open",       label: "All open",    fn: (t) => t.statusKey === "open" },
          { key: "breach",     label: "Breaching",   fn: (t) => t.breaching && t.statusKey !== "resolved", danger: true },
          { key: "resolved",   label: "Resolved",    fn: (t) => t.statusKey === "resolved" },
        ]
      : [
          { key: "mine",     label: "My tickets", fn: (t) => t.requester === me && t.statusKey !== "resolved" },
          { key: "open",     label: "Open",       fn: (t) => t.statusKey !== "resolved" },
          { key: "resolved", label: "Resolved",   fn: (t) => t.statusKey === "resolved" },
        ]
  ), [isAgent, me]);
  ...
  const shown = useMemo(() => {
    let list = tickets.filter(activeView.fn);
    ...
    if (sort === "sla")      sorted.sort((a, b) => (a.dueAt ? new Date(a.dueAt) : Infinity) - (b.dueAt ? new Date(b.dueAt) : Infinity));
    return sorted;
  }, [tickets, activeView, priority, sort]);
      ...
      <div className={id ? "hidden lg:flex" : "flex"}
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <p style={{ color: C.inkSoft, marginTop: 3, fontSize: 13.5 }}>
            {state === "ready" ? `${shown.length} in ${activeView.label.toLowerCase()}` : "Loading your queue…"}
          </p>
        </div>
        ...
        <div className="flex gap-2" style={{ overflowX: "auto", paddingBottom: 10, marginBottom: 4 }}>
          {views.map((v) => {
            ...
            const count = tickets.filter(v.fn).length;
            ...
            return (
              <button key={v.key} onClick={() => setView(v.key)} className="press"
                ...
                {v.label} <span style={{ fontSize: 11, opacity: 0.7 }}>{count}</span>
              </button>
            );
          })}
        </div>
        ...
        {/* Cards (scrolls within the sticky pane) */}
